...
const VideoChat = (props) => {
  ...
  const handleSubmit = useCallback(
    async (event) => {
      ...
      const response = await axios.get(`https://685d8pusc8.execute-api.us-east-1.amazonaws.com/production/generateTokenTwilioVideo?identity=${username}&room=${roomName}`);
      // https://685d8pusc8.execute-api.us-east-1.amazonaws.com/production/generateTokenTwilioVideo?identity=Miho&room=heso
      const { data } = response;
  ...
  if (room) {
    render = (
      <Room roomName={roomName} username={username} room={room} handleLogout={handleLogout} guestRoom={guestRoom} />
    );
  } else {
    render = (
      <Lobby
